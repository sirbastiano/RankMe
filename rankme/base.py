...
def check_tensor_dtype(
    tensor: torch.Tensor, expected_dtype: Union[torch.dtype, tuple[torch.dtype, ...]]
) -> None:
    ...
    if isinstance(expected_dtype, tuple):
        if tensor.dtype not in expected_dtype:
            raise TypeError(
                f"Tensor must have dtype in {expected_dtype}, " f"got {tensor.dtype}"
            )
    else:
...
def to_onehot(
    label_tensor: torch.Tensor, num_classes: int, dtype: Optional[torch.dtype] = None
) -> torch.Tensor:
    ...
    return torch.nn.functional.one_hot(label_tensor.long(), num_classes=num_classes).to(
        dtype
    )
...
def apply_to_collection(
    data: Any, dtype: type, function: Callable[..., Any], *args: Any, **kwargs: Any
) -> Any:
    ...
    if isinstance(data, dtype):
        ...
        result = [
            apply_to_collection(item, dtype, function, *args, **kwargs) for item in data
        ]
